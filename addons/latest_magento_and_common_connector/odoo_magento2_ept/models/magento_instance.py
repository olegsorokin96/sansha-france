...
class MagentoInstance(models.Model):
    ...
    def _default_set_import_product_category(self):
        return self.env.ref('product.product_category_services').id \
            if self.env.ref('product.product_category_services') else False
    ...
    magento_import_attribute_page_count = fields.Integer(string="Magento Import Attribute Page Count",
                                                         ...
                                                         help="It will fetch attribute of Magento "
                                                              "from given page numbers.")
    import_disable_products = fields.Boolean("Import Disabled Products", default=False,
                                             help="If checked, it will also import disabled products from Magento to Odoo. "
                                                  "If not checked, it will not import disabled products from Magento to Odoo.")

    _unique_magento_host = models.Constraint('unique(magento_url, access_token)',
    ...
    def get_refund(self, instance, website_id=0, state=('draft', 'posted', 'cancel')):
        ...
        if website_id:
            ...
            self.env.cr.execute(query, (website_id, instance.id, state))
        else:
            query = self._prepare_refund_query(instance, state)
            self.env.cr.execute(query, (instance.id, state))
    ...
    def get_total_products(self, record, exported, product_type=False):
        ...
        self.env.cr.execute(main_sql, (record.id,))
    ...
    def get_total_orders(self, record, state=False):
        ...
        def orders_of_current_week(record):
            self.env.cr.execute("""select id from sale_order where date(date_order)
    ...
    def get_shipped_orders(self, record):
       ...
       """
        ...
        def shipped_order_of_current_week(shipped_query):
            ...
            self.env.cr.execute(qry, (record.id,))
            return self.env.cr.dictfetchall()
        ...
        def shipped_order_of_all_time(shipped_query):
            self.env.cr.execute(shipped_query, (record.id,))
            return self.env.cr.dictfetchall()
    ...
    def get_magento_invoice_records(self, state):
        ...
        self.env.cr.execute(invoice_query, (self.id, state))
        result = self.env.cr.dictfetchall()
