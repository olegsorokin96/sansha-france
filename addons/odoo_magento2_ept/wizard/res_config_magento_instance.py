...
class ResConfigMagentoInstance(models.TransientModel):
    ...
    def create_magento_instance(self):
        ...
        if self.env.context.get('is_calling_from_magento_onboarding_panel', False):
    ...
    def reset_credentials(self):
        ...
        instance_id = context.get("magento_instance_id")

        instance = magento_instance_obj.browse(instance_id)
        if instance.access_token == self.access_token and context.get("test_connection"):
            raise UserError(_("Entered credentials are same as previous.\nPlease verify the credentials once."))

        instance.write({"magento_url": instance.magento_url,
                        "magento_admin_url": instance.magento_admin_url,
                        "access_token": self.access_token,
                        "magento_verify_ssl": instance.magento_verify_ssl})
        instance.magento_test_connection()
        if context.get("test_connection"):
            return {"type": "ir.actions.client", "tag": "display_notification",
                    "params": {"title": "Magento",
