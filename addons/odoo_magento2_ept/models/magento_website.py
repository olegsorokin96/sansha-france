...
class MagentoWebsite(models.Model):
    ...
    def get_total_products(self, record, exported, product_type=False):
        ...
        self.env.cr.execute(main_sql, (record.id, record.magento_instance_id.id))
    ...
    def get_shipped_orders(self, record):
        ...
        def website_vise_shipped_order_of_current_week(shipped_query):
            ...
            self.env.cr.execute(qry, (record.magento_instance_id.id, record.id))
            return self.env.cr.dictfetchall()
        ...
        def website_vise_shipped_order_of_all_time(shipped_query):
            self.env.cr.execute(shipped_query, (record.magento_instance_id.id, record.id))
            return self.env.cr.dictfetchall()
    ...
    def get_magento_invoice_records(self, state):
        ...
        self.env.cr.execute(magento_invoice_query, (self.id, self.magento_instance_id.id, state))
        result = self.env.cr.dictfetchall()
    ...
    def get_magento_picking_records(self, state):
        ...
        and SP.state in (%s)
        """
        self.env.cr.execute(magento_picking_query, (self.magento_instance_id.id, self.id, state))
        result = self.env.cr.dictfetchall()
        view = \
