...
class SaleOrder(models.Model):
    ...
    def _prepare_invoice(self):
        ...
        elif self.env.context.get('journal_ept'):
            invoice_vals.update({'journal_id': self.env.context.get('journal_ept').id})
        return invoice_vals
    ...
    def prepare_val_for_stock_move_ept(self, product, product_qty, product_uom, vendor_location, customers_location,
        ...
        vals = {
            ...
            'origin': self.name,
            'reference_ids': [Command.set(self.stock_reference_ids.ids)]
        }
        if bom_line:
